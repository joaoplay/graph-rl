...
cdef int bres_segment_count(int x0, int y0, int x1, int y1,
                            float[:, :] grid, int endpoint, source):
...
def bres_segments_count(int[:, :] segments, float[:, :] grid, float[:] sources):
    cdef unsigned k

    for k in range(segments.shape[0]):
        bres_segment_count(segments[k,0], segments[k, 1],
                           segments[k,2], segments[k, 3], grid, 1, sources[k])